...
export const JobCareerHeaderRow = () => {
    return (
        <div className="flex gap-5 pb-4 text-sm md:text-lg">
...
export const JobCareerRow = ({ title="Untitled", avg=0, range=0, rangeMin=0, rangeMax=0 }) => {
    return (
        <div className="flex gap-5 py-2 text-sm md:text-lg">
            <div className="flex-1 p-3 flex items-center">{title}</div>
            <div className="flex-1 p-3 flex items-center">MMK {avg.toLocaleString()} <span className="text-gray-500 pl-1 text-xs md:text-md">per month</span></div>
            <div className="flex-1 p-3">
                <div className="px-2 lg:px-10">
                    <input type="range" min={rangeMin} max={rangeMax} value={range} readOnly title={range.toLocaleString()} />
                </div>
                <div className="flex justify-between">
                    <div className="text-gray-500 text-xs md:text-sm">MMK {rangeMin.toLocaleString()}</div>
                    <div className="text-gray-500 text-xs md:text-sm">MMK {rangeMax.toLocaleString()}</div>
                </div>
            </div>
        </div>
    )
