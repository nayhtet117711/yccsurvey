...
const InputClassName = `block w-full max-w-xs py-2 px-4 my-4 ml-4 md:ml-8 rounded-2xl outline-none border border-violet-600 shadow-md shadow-violet-700 focus:border-violet-700 focus:ring-violet-500 sm:text-xl appearance-none`;
...
const Survey = () => {
  ...
  const handleSubmit = e => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/surveys`, {
      ...
      .then(res => res.json())
      .then(data => {
        removeCookie("survey-data")
        toast.remove(loadingToastId)
        toast.success("Thank you for participating.", { duration: 3000 })
...
const RenderPage1 = ({ surveyData, handleChange }) => {
  const {
    name = "",
    graduated_year = "",
    degree_level = "",
          ...
          <div className="space-y-4 flex flex-col justify-center">
            {graduated_years.map((v) => (
              <div key={v.id} className="flex items-center">
                ...
                />
                <label htmlFor={"graduated_year"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
            ...
            {degree_levels.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"degree_level"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
            ...
            {majors.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"major"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
            ...
            {yes_or_no.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"has_other_degree"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
            ...
            {yes_or_no.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"is_current_college"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
        ...
        <fieldset className="mt-4">
            ...
            {yes_or_no.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"is_current_working"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
          ...
          {is_current_working === "yes" && (
            <div className="pl-4 pt-4">
              <label htmlFor="is_current_working" className={LabelClassName}>Is your own business? *</label>
              <fieldset className="mt-4">
                <legend className="sr-only">Is Current Working?</legend>
                <div className="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                  <div className="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                    {yes_or_no.map((v) => (
                      <div key={v.id} className="flex items-center">
                        <input
                          ...
                          required
                          value={v.id}
                          checked={v.id === is_own_business}
                          onChange={e => handleChange("is_own_business", e.currentTarget.value)}
                          className={RadioClassName}
                        />
                        <label htmlFor={"is_own_business"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                          {v.name}
                        </label>
                      </div>
                    ))}
                ...
                <div className="p-0 pt-4">
                  ...
                  <fieldset className="mt-4">
                    ...
                    <div className="space-y-0 flex items-center space-x-0">
                      <div className="text-xl text-gray-600 pl-2 md:pl-4 md:pl-8">lowest</div>
                      {rating.map((v) => (
                        <div key={v.id} className="flex items-center">
                          <input
                            id={"current_job_rating"+v.id}
                            ...
                            className={RadioClassName}
                          />
                          <label htmlFor={"current_job_rating"+v.id} className="text-xl text-gray-600 hidden">
                            {v.name}
                          </label>
                        </div>
                      ))}
                      <div className="text-xl text-gray-600 pl-2 md:pl-4 md:pl-8">highest</div>
                    </div>
                  </fieldset>
                </div>
                  ...
                  <fieldset className="mt-4">
                    ...
                    <div className="space-y-0 flex items-center space-x-0">
                      ...
                      {rating.map((v) => (
                        <div key={v.id} className="flex items-center">
                          <input
                            id={"job_related_degree_rating"+v.id}
                            ...
                            className={RadioClassName}
                          />
                          <label htmlFor={"job_related_degree_rating"+v.id} className="text-xl text-gray-600 hidden">
                            {v.name}
                          </label>
                        </div>
                      ))}
...
const RenderPage3 = ({ surveyData, handleChange }) => {
  const {
    ...
    <>
      <div className="p-4">
        ...
        <fieldset className="mt-4">
          ...
          <div className="space-y-0 flex items-center space-x-0">
            <div className="text-xl text-gray-600 pl-4 md:pl-8">lowest</div>
            {rating.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  id={"degree_coverage_career_rating"+v.id}
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"degree_coverage_career_rating"+v.id} className="text-xl text-gray-600 hidden">
                  {v.name}
                </label>
              </div>
            ))}
            <div className="text-xl text-gray-600 pl-4 md:pl-8">highest</div>
          </div>
        </fieldset>
      </div>
        ...
        <fieldset className="mt-4">
          ...
          <div className="space-y-0 flex items-center space-x-0">
            ...
            {rating.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  id={"activites_coverage_career_rating"+v.id}
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"activites_coverage_career_rating"+v.id} className="text-xl text-gray-600 hidden">
                  {v.name}
                </label>
              </div>
            ))}
            ...
            {yes_or_no.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"is_internship_helpful"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
            ...
            {yes_or_no.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"is_degree_important"+v.id} className="ml-3 block text-md md:text-xl text-gray-600">
                  {v.name}
                </label>
              </div>
            ))}
            ...
            {rating.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"ict_dept_rating"+v.id} className="text-xl text-gray-600 hidden">
                  {v.name}
                </label>
              </div>
            ))}
        ...
        <fieldset className="mt-4">
          ...
          <div className="space-y-0 flex items-center space-x-0">
            ...
            {rating.map((v) => (
              <div key={v.id} className="flex items-center">
                <input
                  id={"utycc_facilities_rating"+v.id}
                  ...
                  className={RadioClassName}
                />
                <label htmlFor={"utycc_facilities_rating"+v.id} className="text-xl text-gray-600 hidden">
                  {v.name}
                </label>
              </div>
            ))}
